...
echo -e "${BLUE}用法: ./deploy.sh port=9081 target_dir=~/PersonaBook provider=github container_name=persona-book-frontend${NC}"
...
elif [ "$provider" = "gitee" ]; then
    remote=$REMOTE_GITEE
else
    echo -e "${BLUE}未知的svn参数: $provider，使用默认的GitHub仓库${NC}"
    remote=$REMOTE_GITHUB
fi
